    ...
    public static class ProcessUtils
    {
        ...
        public static bool GetMonoModule(IntPtr handle, out IntPtr monoModule)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (path.ToString().IndexOf("mono", StringComparison.OrdinalIgnoreCase) < 0)
                {
                    continue;
                }

                MODULEINFO info;
                if (!Native.GetModuleInformation(handle, ptrs[i], out info, (uint)Marshal.SizeOf(typeof(MODULEINFO))))
                {
        ...
        public static bool Is64BitProcess(IntPtr handle)
        {
            ...
            bool isWow64;
            if (!Native.IsWow64Process(handle, out isWow64))
            {
